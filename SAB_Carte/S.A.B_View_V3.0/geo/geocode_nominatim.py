...
# Récupérer le fichier CSV depuis le .bat
INPUT_FILE = sys.argv[1] if len(sys.argv) > 1 else "adresses.csv"
with open(INPUT_FILE, newline="", encoding="latin1") as fin:
    reader = csv.reader(fin)
    rows = list(reader)

with open("adresses_utf8.csv", "w", newline="", encoding="utf-8") as fout:
    writer = csv.writer(fout)
    writer.writerows(rows)
OUTPUT_FILE = INPUT_FILE.replace(".csv", "_geo.csv")
